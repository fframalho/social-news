        ...
        <Authorized authorizedRoles={['Admin']}>
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="max-w-screen-xl mx-auto mt-10 p-6 bg-white shadow-xl rounded-2xl space-y-6">
                    <h2 className="text-2xl font-bold text-gray-800" data-testid="add-article-title">
                        Add article
                    </h2>
                    <div className="space-y-4">
                        {/* Text Input */}
                        <div>
                            <label className="block mb-1 text-sm font-medium text-gray-700" htmlFor={TITLE_ID}>
                                Title
                            </label>
                            <input
                                id={TITLE_ID}
                        ...
                        {/* Image Upload */}
                        <div>
                            <label className="block mb-1 text-sm font-medium text-gray-700" htmlFor={IMAGE_ID}>
                                Image
                            </label>
                            <input
                                id={IMAGE_ID}
                            ...
                            {imagePreview && (
                                <img src={imagePreview} alt="Preview" className="mt-3 h-24 w-24 object-cover border" />
                            )}
                        </div>

                        {/* Category Select */}
                        <div>
                            <label className="block mb-1 text-sm font-medium text-gray-700" htmlFor={CATEGORY_ID}>
                                Category
                            </label>
                            <select
                                id={CATEGORY_ID}
                        ...
                        {/* State Select */}
                        <div>
                            <label className="block mb-1 text-sm font-medium text-gray-700" htmlFor={STATE_ID}>
                                Save article as:
                            </label>
                            <select
                                id={STATE_ID}
